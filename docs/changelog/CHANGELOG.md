...
## 2026-09-05 服务端接入内网 Gitea（Cy-image-service），main=c16632f / develop 建线，CI 落地

- 服务端仓库接入内网 Gitea 实例「Cy-image-service」：主分支 **main**，当前 main = **c16632f**（`fix(image): switch runtime image endpoint to cf.api.fan`，2026-09-04 17:19，与 GitHub origin/master 同步）；**develop** 建线用于日常开发；**CI 落地**。
