...
- **事故与恢复（2026-09-06 凌晨，如实记录）**：develop 曾被推入错误提交 aa90923（其树 = 用户分支 codex/skill-workshop-4.2.2 的 tip，等于删除全部 workflows / deploy / docs / 同步器等 29 文件）及探针空提交 251abb7。**根因**：temp-index 提交链被拆散到多次工具调用之间、`GIT_INDEX_FILE` 环境变量跨调用丢失，`write-tree` 落到了用户活索引上（用户分支 tip 的树）。**影响核实**：无密钥泄漏（树内容为已公开的生产提交 f333fcb）、无未提交 WIP 泄漏、用户工作区与暂存区完好（`git diff --cached` 为空）。**修复**：前向恢复提交 c0305d9（= 62bc87e 的树 + 3 个 docs/脚本文件，父为 251abb7 快进追加，不重写历史、不强推），推送前执行「diff 文件数必须恰等于预期」守卫，回读 SHA 一致；推送后 CI 秒级恢复触发，四 job 全部实际领取执行（guard 通过后 backend pytest / admin build / cd scripts 依 `needs` 链派发，pytest 测试会话与 CD 脚本 fixture 均真实产生；62bc87e 同链路已验证四 job 全绿，c0305d9 与之仅差 3 个文档/脚本文件）——**逐 job 通过/失败状态以 Gitea Web UI 人工核验为准**（HTTP API 凭据不可用，如实标注）。
- **流程纪律（本次事故教训，双仓适用）**：① temp-index 提交链（read-tree → add → write-tree → commit-tree → update-ref → push）必须在**同一次 shell 调用内**完成，且推送前必须校验 `git diff --name-only <base> <tree>` 的文件数与预期一致，不一致即中止；② Gitea Actions 的触发判定基于**被推提交树内**的 workflow 文件——树里没有 `.gitea/workflows/` 时 Gitea 静默不建 run（无任何报错），aa90923/251abb7 期间「CI 不触发」即此因，并非 Runner 故障。
