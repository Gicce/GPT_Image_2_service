...
def git_info():
    # docs/ 的权威分支是 develop（Gitea）；HEAD 可能停在用户 WIP 分支（不含 docs/ 的提交），
    # 元数据必须指向真正包含这些文档的提交，故优先取 develop/main/master，都不存在才回落 HEAD。
    ...
    if _GIT_INFO_CACHE is None:
        ...
        br = next((b for b in ("develop", "main", "master")
                   if _rev("rev-parse", "--verify", "--quiet", f"refs/heads/{b}")), "")
        if br:
            cm = _rev("rev-parse", "--short", f"refs/heads/{br}") or "unknown"
        else:
            br = _rev("rev-parse", "--abbrev-ref", "HEAD") or "unknown"
            cm = _rev("rev-parse", "--short", "HEAD") or "unknown"
